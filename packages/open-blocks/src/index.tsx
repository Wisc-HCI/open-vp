...
import { DATA_TYPES, combinedBlockData, CLIPBOARD_ACTION, BlockData, useProgrammingStore } from "@people_and_robots/open-core";
...
const Block = memo(
  ({
    ...
    const [data, typeSpec, progress] = useProgrammingStore(
      useCallback(
        (state) => combinedBlockData(state.programData, state.executionData, state.programSpec.objectSpec, staticData, id),
        [id, staticData]
      ),
      stringEquality
    );
...
export {ExternalBlock, PreviewBlock, Block};;
