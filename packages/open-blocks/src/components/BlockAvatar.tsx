...
import { BlockAvatarContainer } from "./BlockContainers";
import {
  useSpring,
  useTransform,
  useAnimationFrame,
...
export const BlockAvatar = ({
  ...
  const timer = useProgrammingStore(
    (state: ProgrammingState) => state.clock,
  ) as Timer;
  ...
  useAnimationFrame(() => {
    ...
    const newProgressValue =
      newProgressState === "indeterminite" ? 0.3 : newProgressState;
    const newStartValue =
      newProgressState === "indeterminite" ? (time % 360) / 360 - 0.05 : 0;
